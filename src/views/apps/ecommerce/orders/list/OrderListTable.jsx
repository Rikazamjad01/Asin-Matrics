...
export const statusChipColor = {
  ...
  UNSHIPPED: { color: 'secondary' },
  PARTIALLY_SHIPPED: { color: 'info' },
  SHIPPED: { color: 'primary' }
}

export const paymentMethodsMap = {
  cod: { text: 'COD', value: 'Cash on Delivery (COD)' }
}
...
const OrderListTable = ({ orderData }) => {
  ...
  const paymentMethodsArray = Object.keys(paymentMethodsMap).map(key => ({
    value: paymentMethodsMap[key].value || key,
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('marketPlace', {
        header: 'Market place',
        cell: ({ row }) => <Typography>{row.original.marketPlace || '—'}</Typography>
      }),
      columnHelper.accessor('fulfillment', {
        header: 'Fulfillment',
        cell: ({ row }) => <Typography>{row.original.fulfillment || '—'}</Typography>
      }),
      ...
      columnHelper.accessor('prime', {
        header: 'Prime',
        cell: ({ row }) => <Typography>{row.original.prime || '—'}</Typography>
      }),
  ...
  return (
    <Card>
          ...
          <DatePicker.RangePicker
            ...
            value={
              filters.startDate && filters.endDate
                ...
                : [dayjs(defaultDateRange.start), dayjs(defaultDateRange.end)]
            }
            onChange={dates => {
              // Date range cannot be removed - always ensure dates are set
              if (dates && dates.length === 2) {
                ...
                // If cleared, restore default range instead of removing
                setFilters(prev => ({
                  ...prev,
                  startDate: defaultDateRange.start,
                  endDate: defaultDateRange.end
      ...
      <OrderDetailsDrawer
        ...
        handleClose={() => setOrderDetailsOpen(false)}
